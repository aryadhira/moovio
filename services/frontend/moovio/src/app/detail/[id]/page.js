            ...
            <div className="p-10 w-1/2">
                ...
                <div className="flex gap-5">
                    {data.qualities.map(quality => {
                        return (
                            <button className="rounded p-2 bg-red-500 w-20 shadow-xl hover:bg-red-400 hover:transition duration-300 ease-in-out hover:scale-125" onClick={() => openModal(`${quality}`)}>{quality}</button>
                        )
                    })}
                </div>
                <div className='pt-10'>
                    <a href='/' className='text-bold underline'>Back to Home</a>
                </div>
            </div>
            {showModal && (
                <Modal onClose={() => setShowModal(false)}>
                    <h1 className='text-white text-2xl font-bold pb-5'>{data.title} - {modalContent} </h1>
